...
print(number)
i= 0  #文件名的序号
fileList_xml = [path+'2016_fulltext_'+str(x) + '.xml' for x in range(1,number_circle+2) ]       #创建文件的名字xml
while (retstart_circle < number):
    handle = Entrez.esearch(db = "pmc",retmax = 100,retstart = retstart_circle,term = "2016[pdat]")
    records = Entrez.read(handle)
    handle.close()
    b = records["IdList"]     #获取具体的UID
